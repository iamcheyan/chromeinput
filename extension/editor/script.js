...
const IME_DICT_PATHS_STORAGE_KEY = (window.CIShared && CIShared.KEYS.DICT_PATHS) || 'ci_dict_paths';
const GLOBAL_ENABLED_STORAGE_KEY = 'ci_enabled';
...
const EDITABLE_DICT_PATHS = window.CI_DICTS?.EDITOR_PATHS || [];
const IME_DICT_TABLES = window.CI_DICTS?.TABLES || [];
const DEFAULT_IME_DICT_PATHS = window.CI_DICTS?.DEFAULT_PATHS || IME_DICT_TABLES.map((table) => table.path);
const CONTENT_IME_SCRIPT_CHAIN = [
  ['../shared/dictload.js', 'CIDictLoad'],
  ['../content/engine.js', 'CIEngine'],
...
async function ensureCIShared() {
  // shared/config.js + storage.js + toast.js 由 index.html 静态引入
  if (!window.CIShared) {
    throw new Error('CIShared unavailable (storage.js/toast.js not loaded)');
  }
  return window.CIShared;
}

async function ensureContentIME() {
  await ensureCIShared();
  if (window.CIContentIME) return window.CIContentIME;
  window.__CI_CONTENT_AUTO_INIT__ = false;
  try {
    for (const [path] of CONTENT_IME_SCRIPT_CHAIN) {
      // 逐文件加载: 不能按全局名短路 (多个模块共写 CIContent)
      if (!sharedScriptPromises.has(path)) {
        const promise = new Promise((resolve, reject) => {
          const script = document.createElement('script');
          script.src = path;
      ...
      await sharedScriptPromises.get(path);
    }
    if (!window.CIContentIME) {
      throw new Error('Content IME chain loaded but CIContentIME unavailable');
    }
  } finally {
    delete window.__CI_CONTENT_AUTO_INIT__;
  }
  return window.CIContentIME;
}
...
async function reinstallImeController() {
  ...
  const install = previousInstall.catch(() => {}).then(async () => {
    if (workspace.featureIme === false) {
      if (ciImeController?.destroy) ciImeController.destroy();
      ciImeController = null;
      return;
    }

    const CIContentIME = await ensureContentIME();
    if (version !== imeInstallVersion || workspace.featureIme === false || !CIContentIME?.installTextareaIME) return;

    // 词库选择由 content 侧直接读 storage (ci_dict_paths), 此处仅传目标与抑制检查
    if (ciImeController && ciImeController.destroy) ciImeController.destroy();
    ciImeController = CIContentIME.installTextareaIME({
      target: editor,
      isSuppressed: () => !!(
...
function ensureImeControllerIfNeeded() {
  if (workspace.featureIme === false || ciImeController || imeInstallPromise) return;
...
function saveWorkspace() {
  ...
  try {
    ...
    console.warn('CI: Workspace persistence skipped.', error);
  }
}
...
function syncFeatureVisibility() {
  ...
  // 内置字体特性已随 SarasaMonoSC 字体移除 (GOAL §四), featureFont 仅为兼容旧工作区保留
  workspace.featureFont = false;
  ...
  if (!workspace.featureIme && ciImeController?.destroy) {
    ciImeController.destroy();
    ciImeController = null;
  }
...
async function reloadDeferredSourceTab(tab) {
  ...
  const reload = (async () => {
    try {
      const CIShared = await ensureCIShared();
      const content = await CIShared.readDictResource(tab.sourcePath);
      tab.content = content;
      ...
      console.error(`CI: Failed to reload ${tab.sourcePath}.`, error);
...
async function openDictFile(path) {
  ...
  const content = await CIShared.readDictResource(path);
...
async function saveCurrentSourceTab() {
  ...
  // JSON 词库保存前校验; native host 已移除, 统一 storage override (GOAL §一)
  if (tab.sourcePath.endsWith('.json')) {
    if (!CIShared.parseEntries(editor.value)) {
      CIShared.showAppToast('Invalid JSON dictionary: expected [[word, code, weight], ...]', { tone: 'error' });
  ...
  try {
    await CIShared.saveDictOverride(tab.sourcePath, editor.value);
  } catch (error) {
    console.error('CI: Save failed.', error);
    setSaveButtonFeedback('is-error', 'Save failed', SAVE_ERROR_FEEDBACK_MS);
    return;
  }
...
async function promptAndSaveFixedEntry(selectedText) {
  const CIShared = await ensureCIShared();
  await CIShared.promptAndSaveShortcutEntry(selectedText, {
...
settingFeatureIme.addEventListener('change', () => {
  ...
  } else if (ciImeController?.destroy) {
    ciImeController.destroy();
    ciImeController = null;
  }
...
// setting-feature-font 已从设置面板移除 (字体随零依赖规范一并移除)
...
for (const path of EDITABLE_DICT_PATHS) {
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error(`CI: Failed to open ${path}.`, error);
      const CIShared = await ensureCIShared();
      CIShared.showAppToast(`Unable to open ${getDictFileLabel(path)}.`, { tone: 'error' });
    } finally {
...
if (chrome.runtime?.onMessage) {
  chrome.runtime.onMessage.addListener((message) => {
    if (message?.type === 'ci_add_selection_to_fixed_dict') {
      void promptAndSaveFixedEntry(message.text || '');
      return;
    }
    if (message?.type === 'ci_add_current_selection_to_fixed_dict') {
      void ensureCIShared().then((CIShared) => {
        void promptAndSaveFixedEntry(CIShared.getActiveSelectedText());
      });
    }
  });
}
...
if (chrome.storage?.onChanged) {
  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === 'local') {
      ...
      if (changes[IME_DICT_PATHS_STORAGE_KEY] && ciImeController) {
        void reinstallImeController();
      }
    }
  });
...
tabsRoot.addEventListener('pointerdown', () => {
  ...
  void ensureCIShared().then((CIShared) => {
    if (tabDraggingInstalled || !CIShared?.installTabDragging) return;
    CIShared.installTabDragging(tabsRoot, {
