(function (global) {
  class VimMode {
    ...
    yankLine() {
      ...
      let end = text.indexOf('\n', this.pos);
      if (end === -1) end = text.length;
      // chromeinput: 行级寄存器统一带换行 (源仓末行不带, 与 dd 不一致), 配合下行级 p
      this.clipboard = text.slice(start, end) + '\n';
    }
    ...
    paste() {
      ...
      const text = this.editor.value;
      let p = this.pos;
      // chromeinput: 行级寄存器 p 按 vim 语义插到当前行之后 (源仓为光标处内联且丢尾部,
      // 属上游 bug), 见 docs/REFACTOR_NOTES.md 行为存疑清单.
      if (this.clipboard.endsWith('\n')) {
      ...
      this.editor.value = text.slice(0, p) + this.clipboard + text.slice(p);
